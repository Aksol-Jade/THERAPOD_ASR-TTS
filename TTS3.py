...
def output_with_piper(text, output_file):
    try:
        ...
        command = f'echo "{text}" | .\\piper.exe -m en_US-lessac-high.onnx -f {output_file}'
...
def main():
    ...
    while True:
        user_input = input("Enter the text you want Piper to speak (or type 'exit' to quit): ")
        
        if user_input.lower() == 'exit':
            print("Exiting the program.")
            break
        
        output_with_piper(user_input, current_output_file)

        # Switch to the other output file for the next iteration
        current_output_file = output_file_2 if current_output_file == output_file_1 else output_file_1
